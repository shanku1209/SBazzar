...
function ProductsScreen(props) {
  ...
  <div className="content content-margined">
    ...
    <form onSubmit={submitHandler} >
      <ul className="form-container">
        ...
        <li>
          <label htmlFor="countIStock">
            Count In Stock
          </label>
          <input type="text" name="coutInStock" value={countInStock} id="countInStock" onChange={(e) => setCountInStock(e.target.value)}>
          </input>
        </li>
        <li>
    ...
    <div className="product-list">
      <table className="table">
        ...
        {products.map((product)=>(<tr key={product._id}>
          ...
          <td>{product.brand}</td>
        <td>  <button className="button" onClick={()=>openModal(product)} >
                    Edit
                  </button>
                  {'   '}
  ...
  return ( <div>
    <div>
      <h1 className='profilename'>Welcome {userInfo.name}</h1>
    </div>
</div>
)
